...
namespace ex_5_1
{
  ...
  void driveDistance(float distanceCm, float power)
  {
    ...
    leftCounter = 0;
    startMotor(static_cast<int>(power));
    runStartTime = millis();
  ...
  void do_ex()
  {
    if (stage == 0 && distanceDriven)
    {
      driveDistance(20, maxPower * 0.5);
      stage++;
    }
    if (stage == 1 && distanceDriven)
    {
      driveDistance(5, maxPower * 0.25);
      stage++;
    }
    if (stage == 2 && distanceDriven)
    {
      driveDistance(10, maxPower * 0.75);
      stage++;
      do_run = false;
    }
  }
        ...
        .withChangeInterrupt(
            makeCallback(
                []()
                {
                  ...
                  if (leftCounter >= targetEncodings)
                  {
                    stopMotor();
                    distanceDriven = true;
